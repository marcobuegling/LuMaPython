...
def _outer(array1:np.ndarray, array2:np.ndarray) ->  np.ndarray:
    return np.multiply.outer(array1, array2)
'''

@vectorize([float64(float64, float64)])
def _mult(array1:np.ndarray, array2:np.ndarray) ->  np.ndarray:
...
class NashGrid:
    ...
    def solveWithLogitEquilibrium(self) -> Tuple[np.ndarray, dict, dict]:
        return NashGrid._solveWithLogitEquilibrium(self._contents)
    
    
        
    @staticmethod
    #@njit   
    def _solveWithLogitEquilibrium(contents: np.ndarray) -> Tuple[np.ndarray, dict, dict]:
        ...
        for playerId in range(playerCount):
            playerProbabilitiesDict[playerId] = np.full(shape=(contents.shape[playerId+1]), fill_value=1.0/contents.shape[playerId+1])
            playersExpectedValuesDict[playerId] = np.zeros(shape=(contents.shape[playerId+1]))
        ...
        for iteration in range(ITERATIONS):
            ...
            for playerId in range(playerCount):
                ...
                eu_tensor = contents[playerId]
                p_tensor = np.ones(shape=(contents.shape[playerId+1])) if playerId == 0 else playerProbabilitiesDict[0]
                for intern_player_id in range(1, playerCount):
                    other_p_tensor = np.ones(shape=(contents.shape[playerId+1])) if intern_player_id == playerId else playerProbabilitiesDict[intern_player_id]
                    p_tensor = _mult.outer(p_tensor, other_p_tensor)
                    
                
                # recalculate the expected value each iteration using the new probabilities
                new_eu_tensor = p_tensor*eu_tensor
                
                # construct a shape, that simply contains all player ids as a tuple, except the player id of the current player
                specific_axis_shape = list(range(playerCount))
                specific_axis_shape.remove(playerId)
                specific_axis_shape = tuple(specific_axis_shape)
